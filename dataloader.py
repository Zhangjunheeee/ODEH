...
class Dataloader:
    ...
    def _get_iter(self, start_idx=0):
        ...
        for i in range(start_idx, len(self.ds)):
            if B is None:
                t, dt, B, delta_B, users, items, feat_tuple, states = self.ds.getitem(i, False)
            else:
                B += delta_B
                t, dt, _, delta_B, users, items, feat_tuple, states = self.ds.getitem(i, True)
            ...
            feat_umat = sparse_mx_to_torch_sparse_tensor(feat_umat).to(self.device)
            feat_imat = sparse_mx_to_torch_sparse_tensor(feat_imat).to(self.device)
            feat_tuple = (feats, feat_umat, feat_imat)
            if states is not None:
                states = torch.from_numpy(states).long().to(self.device)
            yield t, dt, adj, i2u_adj, u2i_adj, users, items, feat_tuple, states
...
class Dataset:
    ...
    def __init__(self, df, features, n_users, n_items, t0=0., adj0=None):
        ...
        #self.unique_ts, self.cum_n_records = self.process_timestamps(df.iloc[:, 3])
    ...
    def getitem(self, idx, only_delta=True):
        ...
        umat, imat = self.build_feat_mats(self.df.iloc[a:b])
        feat_tuple = (feats, umat, imat)
        if self.df.shape[1] > 3:
            states = self.df.iloc[a:b, 3].values
        else:
            states = None
        return t, dt, observed_mat, delta_mat, users, items, feat_tuple, states
    ...
    def build_feat_mats(self, df):
        n = len(df)
        u = df.iloc[:, 0]
        i = df.iloc[:, 1]
        umat = sp.csc_matrix((np.ones(n), (u, np.arange(n))), shape=[self.n_users, n])
        imat = sp.csc_matrix((np.ones(n), (i, np.arange(n))), shape=[self.n_items, n])
        umat = sp.diags(inverse_degree_array(np.array(umat.sum(1))).squeeze()) @ umat
        imat = sp.diags(inverse_degree_array(np.array(imat.sum(1))).squeeze()) @ imat
        return umat, imat
